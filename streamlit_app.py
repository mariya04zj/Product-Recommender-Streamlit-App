...
for i, row in paginated_data.iterrows():
    image_path = os.path.join("all_images/all_images", row['image_name'])
    with cols[i % 4]:
...
if st.session_state.selected_image_name:
    ...
    selected_image_path = os.path.join("all_images/all_images", selected_image_name)
    ...
    for i, row in recommendations.iterrows():
        # Get the image path
        recommended_image_path = os.path.join("all_images/all_images", row['image_name'])
